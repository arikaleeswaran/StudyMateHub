...
function LeaderboardPage() {
  ...
  return (
    <div style={{ width: '100vw', minHeight: '100vh', background: 'radial-gradient(circle at top, #1e293b, #0f172a)', color: 'white', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ maxWidth: '800px', margin: '40px auto', padding: '20px', width: '90%' }}>
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
            <h1 style={{ fontSize: '3rem', margin: '0 0 10px 0' }}>🏆 Hall of Fame</h1>
            <p style={{ color: '#aaa', fontSize: '1.2rem' }}>Top students mastering the tech world.</p>
        </div>
                    ...
                    return (
                        <div key={user.user_id} style={{
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                                ...
                                <div>
                                    <h3 style={{ margin: 0, fontSize: '1.2rem' }}>{user.full_name}</h3>
                                    <span style={{ fontSize: '0.9rem', color: '#aaa' }}>{index < 3 ? "Top Performer" : "Rising Star"}</span>
                                </div>
                            </div>
                            <div style={{ textAlign: 'right' }}>
                                <h2 style={{ margin: 0, color: style.color }}>{user.score} pts</h2>
                            </div>
                        </div>
                    );
                })}
